...
function formatFermentableItem(f: Fermentable, index?: number): string {
...
function formatStockFermentable(f: Fermentable): string {
  if (f.inventory == null) return "Unknown";
  if (f.inventory < 0) return `${formatWeight(f.inventory)} (negative — check records)`;
  if (f.inventory === 0) return "Out of stock";
  return formatWeight(f.inventory);
}

// ── Hops ──────────────────────────────────────────────────────────────────────

function formatHopAroma(h: Hop): string {
  if (!h.aroma) return "";
  const traits = [
...
function formatHopItem(h: Hop, index?: number): string {
  const prefix = index != null ? `[${index + 1}] ` : "";
...
function formatStockHop(h: Hop): string {
  if (h.inventory == null) return "Unknown";
  if (h.inventory < 0) return `${h.inventory.toFixed(0)} g (negative — check records)`;
  if (h.inventory === 0) return "Out of stock";
  return `${h.inventory.toFixed(0)} g`;
}

// ── Yeasts ────────────────────────────────────────────────────────────────────

function formatYeastItem(y: Yeast, index?: number): string {
  const prefix = index != null ? `[${index + 1}] ` : "";
...
function formatStockYeast(y: Yeast): string {
  if (y.inventory == null) return "Unknown";
  if (y.inventory < 0) return `${y.inventory} ${y.unit} (negative — check records)`;
  if (y.inventory === 0) return "Out of stock";
  return `${y.inventory} ${y.unit}`;
}

// ── Miscs ─────────────────────────────────────────────────────────────────────

function formatMiscItem(m: Misc, index?: number): string {
  const prefix = index != null ? `[${index + 1}] ` : "";
...
function formatStockMisc(m: Misc): string {
  if (m.inventory == null) return "Unknown";
  if (m.inventory < 0) return `${m.inventory} ${m.unit} (negative — check records)`;
  if (m.inventory === 0) return "Out of stock";
...
export function formatInventoryItem(
  ...
  switch (type) {
    case "fermentables":
      return formatFermentableItem(item as Fermentable, index);
    case "hops":
      return formatHopItem(item as Hop, index);
    case "yeasts":
      return formatYeastItem(item as Yeast, index);
    case "miscs":
      return formatMiscItem(item as Misc, index);
    default:
      return JSON.stringify(item);
  }
}

export function formatInventoryList(items: InventoryItem[], type: string): string {
  if (items.length === 0) return `No ${type} found in inventory.`;
  const header = `Found ${items.length} ${type}:\n`;
  const lines = items.map((item, i) => formatInventoryItem(item, type, i)).join("\n");
  return header + "\n" + lines;
}

export function formatInventoryUpdateConfirmation(
  item: InventoryItem,
  type: string
): string {
